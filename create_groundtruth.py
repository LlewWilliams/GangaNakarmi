...
We ran in Visual Studio Code in a conda environment 
'''
...
from imageio import imread as imageio_imread, imwrite
...
##################################
# we are not running from command line, so initialization is required
win_width = 800
# This is wehere we set up information for our images. Make sure you have a desired image in the folder. 
# individual image
image_name = "IMAGE_NAME_HERE.jpg"
input_image_path = 'PATH_RELATIVE_TO_THIS_FILE'+image_name

#output_mat_file_path 
output_mat_file_path = "PATH_RELATIVE_TO_THIS_FILE"+image_name
png_output_location = 'PATH_RELATIVE_TO_THIS_FILE'
tile = 224 #96 #128  

#output path for tiles
outpath = 'PATH_RELATIVE_TO_THIS_FILE'
# tiles are here
os.path.basename("/path/to/some/file.txt").split('.')[0]
...
def writeout(tmp, cl, labels, outpath, thres):
   ...
   if cnt/len(cl.flatten()) > thres:
      shortname = image_name.split('.')[0]
      outfile = shortname + '_' + id_generator()+'.jpg'
      outpath = outpath+labels[l]+'/'+outfile
      flippedimg = cv2.cvtColor(tmp, cv2.COLOR_BGR2RGB)
      imwrite(outpath, flippedimg)
...
#########################################
#This is the setting up a function for using mouse to pick classes in the tiles.
#==============================================================================
...
#==============================================================================
########################################
#Here is where execution of code starts 
#==============================================================
if __name__ == '__main__':
   ...
   for opt, arg in opts:
      if opt == '-h':
         ...
         win = arg
   ##################################
   # Here is a setting up the size of window and the brush to work with
   #===============================================
   win = win_width
   win = int(win) ##1000
   ...
   thres = .9
   ###############################
   # We defined the labeks and color for the classes
   labels = ['anthro','forest','grass','road','rock','sky','water']
   cmap = ['#CC00CC','#2E7D32','#DCE775','#0000cc','#FF0000','#00FFFF','#154360']
   ...
   #this is the original scipy version 
   ...
   for ck in range(len(Z)):
      ...
      if np.std(im)>0:
         ###########################
         # We are going to loop through seven labels 
         for label in labels:
         ...
         out[Zx[ck],Zy[ck]] = Lc
      ##################### Magic happens. A bunch of array created  
   ...
   ##############################
   # calling CRF function
   print('Generating dense scene from sparse labels ....')
   ############ Magic happens
   res,p = getCRF_justcol(im, Lc, theta, n_iter, classes, compat_col, scale)

   savemat(output_mat_file_path.split('.')[0]+'_mres.mat', {'sparse': Lc.astype('int'), 'class': res.astype('int'), 'preds': p.astype('float16'), 'labels': labels}, do_compression = True)

   img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
   #=============================================
   name, ext = os.path.splitext(image_path)
   name = name.split(os.sep)[-1]
   print('Generating plot ....') ######## Creating a plot
   fig = plt.figure()
   ...
   _ = ax1.imshow(img)
   plt.title('a) Input', loc='left', fontsize=6) ##### labeling the portion as Input
   ax1 = fig.add_subplot(132)
   ax1.get_xaxis().set_visible(False)
   ax1.get_yaxis().set_visible(False)

   _ = ax1.imshow(img)
   plt.title('b) Unary potentials', fontsize=6)####### labeling the portion as Unary potential
   ...
   _ = ax1.imshow(img)
   plt.title('c) CRF prediction', loc='left', fontsize=6) ###### labeling the portion as CRF
   ...
   cb.ax.set_yticklabels(labels)
   cb.ax.tick_params(labelsize=4)
   plt.savefig(png_output_location + name+'_mres.png', dpi=600, bbox_inches='tight')
   del fig; plt.close()
